...
module Btn_Debounce (
    ...
    // Parameter
    parameter COUNT = 100_000, WIDTH = $clog2(COUNT), SHIFT = 10;
    // Reg & Wire
    reg  [SHIFT-1:0] rReg;
    reg  [SHIFT-1:0] rNext;
    reg              rBtn;
    ...
    // Shift_Register
    always @(posedge rDB_Clk, posedge iRst) begin
        if (iRst) rReg <= 0;
        else rReg <= rNext;
    end
    ...
    always @(posedge iClk, posedge iRst) begin
        if (iRst) rEdge_reg <= 1'b0;
        else rEdge_reg <= wDebounce;
    end
